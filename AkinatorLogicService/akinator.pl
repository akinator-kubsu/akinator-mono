...
has_attribute(sponch_bob, works_at_restaurant).
has_attribute(sponch_bob, works_as_fry_cook).
has_attribute(sponch_bob, wears_square_pants).
has_attribute(sponch_bob, has_a_pet_snail_named_gary).
has_attribute(sponch_bob, is_optimistic).
has_attribute(sponch_bob, from_nickelodeon).
...
% Заяц (Ну, погоди!)
has_attribute(zayac_nu_pogodi, animated). % <-- ИСПРАВЛЕНО
...
has_attribute(zayac_nu_pogodi, is_mostly_silent). % <-- ИСПРАВЛЕНО ИМЯ
...
main :-
    ...
    ( Argv = [Input],
      atom_to_term(Input, Query, _),
      ( call(Query) -> format('~w', [Query]) ; write('not_found') ),
      halt
